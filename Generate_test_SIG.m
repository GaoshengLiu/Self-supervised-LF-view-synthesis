...
for DatasetIndex = 1 : datasetsNum
    ...
    for iScene = 1 : sceneNum
        ...
        [U, V, H, W, ~] = size(LF);
        label  = single(zeros(H, W, angRes_label*angRes_label-angRes*angRes));%single类型占4个字节angRes
...
%         label  = single(zeros(H, W, angRes_label, angRes_label));%single类型占4个字节angRes
%         
%         count = 0;
%         for u = 1 : U
%             for v = 1 : V
...
%         label = permute(label,[2,1,4,3]); %[h,w,u,v] -> [w,h,v,u,N]  
        ...
        data  = single(zeros(Hl, Wl, angRes, angRes));%single类型占4个字节angRes
        for u = 1 : Ul
            for v = 1 : Vl
